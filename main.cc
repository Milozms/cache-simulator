...
int main(int argc, char* argv[]) {
    ...
    for (int cachesize = 32768; cachesize <= (32768 << 10); cachesize <<= 1) {
        //printf("Cache size = %d KB:", cachesize >> 10);
        Memory *m = new Memory();
        Cache *l1 = new Cache(cachesize, ass, cachesize / ass / blocksize, 0, 1, 3, m);
        StorageStats s;
        s.access_time = 0;
        s.access_counter = 0;
        s.fetch_num = 0;
